 ...
 *
 * Every operation accepts a PoolClient so callers control the
...
export async function lockWallet(client: PoolClient, userId: string): Promise<WalletState> {
  await client.query(
...
async function readWallet(client: PoolClient, userId: string): Promise<WalletState> {
  const res = await client.query(
...
export const ledgerService = {
  ...
  async getWallet(userId: string, client?: PoolClient): Promise<WalletState> {
